...
void displayMaze (size_t n_rows, size_t n_columns, qType array[n_rows][n_columns]);
...
_Bool solveMazeBFS(char *fileName)
{
  ...
  //      size_t row     = countLinesOfFile  (fileName);
  //      size_t column  = countColumnsOfFile(fileName);
...
void displayMaze (size_t n_rows, size_t n_columns, qType arr[n_rows][n_columns])
{
    size_t y, x;
	for (y = 0; y < n_rows; y++) {
